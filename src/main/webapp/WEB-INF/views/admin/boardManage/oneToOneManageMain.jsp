...
<body>
	...
	<c:if test="${ sessionScope.loginUser.user_type eq 'ADMIN' }">
		...
		<div class="lowerPage">
			<h2>
				<span class="label label-primary">1:1 상담 목록</span>
			</h2>
			<div class="table-responsive">
				<table class="table table-hover">
					...
					<tbody>
						<c:forEach var="row" items="${ oneToOneList }">
							<tr>
				...
				[이전] &nbsp;
					...
					<c:if test="${ page.currentPage > 1}">
						<c:url var="blistBack" value="/admin/oneToOneManage.admin">
					...
					<c:forEach var="p" begin="${ page.startPage }"
						...
						<c:if test="${ p ne page.currentPage }">
							<c:url var="blistCheck" value="/admin/oneToOneManage.admin">
				...
				&nbsp; [다음]
					...
					<c:if test="${ page.currentPage < page.maxPage }">
						<c:url var="blistEnd" value="/admin/oneToOneManage.admin">
							<c:param name="currentPage" value="${ page.currentPage + 1 }" />
						</c:url>
